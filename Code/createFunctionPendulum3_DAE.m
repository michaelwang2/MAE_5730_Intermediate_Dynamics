function createFunctionPendulum3_DAE()
% create RHS for 3-linked pendulum using the DAE method
% create symbols
...
syms R0x R0y R1x R1y R2x R2y                          real
...
F3x = R2x + m3*g;
F3y = R2y;
M3 = cross(R_B32I*[-l3/2; 0; 0], [R2x; R2y; 0]);
M3 = M3(3);
...
% form equations
vars = [x1dd; y1dd; x2dd; y2dd; x3dd; y3dd; thdd1; thdd2; thdd3; R0x; R0y; R1x; R1y; R2x; R2y];
A = jacobian(eqs.', vars);
b = -(eqs.' - A*vars);
vec = simplify(A\b);
...
fid = fopen('pendulum_DAE_3.m', 'w');
fprintf(fid, 'function zdot = pendulum_DAE_3(t,z,p)\n');
